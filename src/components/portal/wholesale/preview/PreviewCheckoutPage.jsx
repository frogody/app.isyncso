...
export default function PreviewCheckoutPage({ config, cart, nav }) {
  ...
  // Submit order -- calls edge function to create a B2B order (bypasses client-side auth issues)
  // (defined before handleVerifyOTP so it can be referenced in its deps)
  const handleSubmitOrder = useCallback(async () => {
    console.log('[Checkout] handleSubmitOrder called', { isAuthenticated, clientId: client?.id, orgId });
    ...
    try {
      ...
      console.log('[Checkout] Calling b2b-create-order edge function...');

      // Call edge function (service_role key, bypasses RLS)
      const res = await fetch(`${SUPABASE_URL}/functions/v1/b2b-create-order`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          clientNotes: notes,
          paymentTermsDays: client.payment_terms_days || 30,
          poNumber: poNumber || null,
          items: orderItems,
        }),
      });

      console.log('[Checkout] Edge function response status:', res.status);
