...
export default function PreviewCheckoutPage({ config, cart, nav }) {
  ...
  const handleSubmitOrder = useCallback(async () => {
    ...
    if (!isAuthenticated || !client?.id || !orgId) {
      console.error('[Checkout:DEBUG] Auth guard failed', { isAuthenticated, clientId: client?.id, orgId });
    ...
    try {
      ...
      console.log('[Checkout:DEBUG] Creating order with payload:', JSON.stringify(orderPayload));

      // Create the order (order_number + po_number auto-generated by DB triggers)
      const order = await createB2BOrder(orderPayload);

      console.log('[Checkout:DEBUG] Order created successfully:', {
        id: order.id,
      ...
      if (items.length > 0) {
        ...
        console.log('[Checkout:DEBUG] Creating order items:', orderItems.length);
      ...
      console.error('[Checkout:DEBUG] ORDER CREATION FAILED:', err);
      console.error('[Checkout:DEBUG] Error name:', err?.name);
      console.error('[Checkout:DEBUG] Error message:', err?.message);
      console.error('[Checkout:DEBUG] Error code:', err?.code);
      console.error('[Checkout:DEBUG] Error details:', err?.details);
      console.error('[Checkout:DEBUG] Error hint:', err?.hint);
      console.error('[Checkout:DEBUG] Full error:', JSON.stringify(err, Object.getOwnPropertyNames(err)));
      setSubmitError(err.message || 'An unexpected error occurred. Please try again.');
    } finally {
      console.log('[Checkout:DEBUG] handleSubmitOrder finally block');
      setSubmitting(false);
    }
  ...
  const handleVerifyOTP = useCallback(async () => {
    ...
    console.log('[OTP:DEBUG] handleVerifyOTP called', { code, email: client?.email, orgId });
    if (code.length !== 6 || !client?.email || !orgId) {
      console.warn('[OTP:DEBUG] Guard failed — code length or missing email/orgId');
      return;
    }
    setOtpVerifying(true);
    setOtpError(null);
    try {
      console.log('[OTP:DEBUG] Calling verify endpoint...');
      const res = await fetch(`${SUPABASE_URL}/functions/v1/b2b-checkout-otp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      console.log('[OTP:DEBUG] Verify response status:', res.status);
      const data = await res.json();
      console.log('[OTP:DEBUG] Verify response data:', JSON.stringify(data));
      if (data.verified) {
        console.log('[OTP:DEBUG] OTP verified! Calling handleSubmitOrder...');
        setOtpVerified(true);
        setOtpVerifying(false);
        await handleSubmitOrder();
        console.log('[OTP:DEBUG] handleSubmitOrder completed');
      } else {
        console.warn('[OTP:DEBUG] OTP NOT verified:', data);
        setOtpAttemptsRemaining(data.attemptsRemaining ?? 0);
        setOtpError(data.message || 'Incorrect code');
        setOtpCode(['', '', '', '', '', '']);
      }
    } catch (err) {
      console.error('[OTP:DEBUG] handleVerifyOTP error:', err);
