  ...
  return (
    <div className={cn("space-y-8", className)}>
      ...
      {showToggle && (
        <div className="flex items-center justify-center gap-4">
          <span className={cn(
            ...
            billingCycle === 'monthly' ? t('text-slate-900', 'text-white') : t('text-slate-500', 'text-zinc-500')
          )}>
            Monthly
          </span>
          <Switch
          ...
          <span className={cn(
            ...
            billingCycle === 'yearly' ? t('text-slate-900', 'text-white') : t('text-slate-500', 'text-zinc-500')
          )}>
            Yearly
          ...
          return (
            ...
            >
              ...
              <div className="p-6 pt-8">
                <h3 className={`text-xl font-semibold ${t('text-slate-900', 'text-white')} mb-2`}>
                  {pkg.name}
                </h3>
                <p className={`text-sm ${t('text-slate-500', 'text-zinc-500')} mb-6 min-h-[2.5rem]`}>
                  {pkg.description || 'Perfect for getting started'}
                </p>

                <div className="mb-6">
                  <div className="flex items-baseline gap-2">
                    <span className={`text-4xl font-bold ${t('text-slate-900', 'text-white')}`}>
                      ${parseFloat(price).toLocaleString()}
                    </span>
                    <span className={t('text-slate-500', 'text-zinc-500')}>
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    isPopular
                      ...
                      : `${t('bg-slate-100 hover:bg-slate-200 text-slate-900', 'bg-white/10 hover:bg-white/20 text-white')}`
                  )}
                >
                    ...
                    return (
                      <div key={i} className="flex items-start gap-3">
                        {isIncluded ? (
                          ...
                          <X className={`w-5 h-5 ${t('text-slate-300', 'text-zinc-600')} flex-shrink-0 mt-0.5`} />
                        )}
                        <span className={cn(
                          "text-sm",
                          isIncluded ? t('text-slate-700', 'text-zinc-300') : t('text-slate-400', 'text-zinc-600')
                        )}>
                          {featureText}
                        </span>
                        {tooltip && (
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger>
                                <HelpCircle className={`w-4 h-4 ${t('text-slate-400', 'text-zinc-600')}`} />
                              </TooltipTrigger>
                              <TooltipContent className={`${t('bg-white', 'bg-zinc-800')} ${t('border-slate-200', 'border-zinc-700')} ${t('text-slate-900', 'text-white')}`}>
                                {tooltip}
                              </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
                ...
                {pkg.limits && (
                  <div className={`mt-6 pt-4 border-t ${t('border-slate-200', 'border-white/5')} space-y-2`}>
                    {Object.entries(pkg.limits).map(([key, value]) => (
                      <div key={key} className="flex items-center justify-between text-sm">
                        <span className={`${t('text-slate-500', 'text-zinc-500')} capitalize`}>
                          {key.replace(/_/g, ' ')}
                        </span>
      ...
      <div className="text-center pt-8">
        <p className={`${t('text-slate-500', 'text-zinc-500')} mb-4`}>
