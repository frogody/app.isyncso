...
export default function CandidateCard({
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        console.log(`[CandidateCard] 🔄 Hydrating missing ${stage} message from server for candidate ${candidate.id}...`);
        ...
        if (!m) {
          console.warn(`[CandidateCard] ⚠️ Candidate ${candidate.id} not found in fresh campaign data during FU hydration.`);
          return;
        }

        if (needsFU1 && m.follow_up_message && !done) {
          setOutreachMessage(m.follow_up_message);
          console.log('[CandidateCard] ✅ FU1 hydrated from server');
          done = true;
        }
        if (needsFU2 && m.follow_up_message_2 && !done) {
          setOutreachMessage(m.follow_up_message_2);
          console.log('[CandidateCard] ✅ FU2 hydrated from server');
          done = true;
        }
      } catch (e) {
  ...
  const handleRegenerateOutreach = async () => {
    ...
    try {
      ...
      if (newMessage) {
        console.log(`✅ New message generated for ${stage}:`, newMessage.substring(0, 100));
        setOutreachMessage(newMessage);

        // CRITICAL: Fetch fresh campaign data
        console.log('🔄 Fetching fresh campaign data before regenerate save...');
        ...
        console.log('💾 Campaign updated with new message');
  ...
  const handleSendToAgent = async () => {
    ...
    try {
      console.log('🤖 Sending to agent:', candidate.id);
      console.log('📊 Current stage:', stage);
      console.log('📧 Outreach message length:', outreachMessage.length);
      ...
      console.log('📤 Response from createOutreachTask:', response);

      const responseData = response?.data || response;

      if (responseData?.success) {
        console.log('✅ Task created:', responseData.task?.id);
        console.log('📊 Task status:', responseData.task?.status);
        ...
        console.log('🔄 Fetching fresh campaign data before saving task/stage updates...');
        const freshCampaigns = await Campaign.filter({ id: campaign.id });
        const freshCampaign = freshCampaigns[0];
        ...
        console.log(`💾 Updating campaign, moving to ${nextStage} stage`);
        await Campaign.update(campaign.id, { matched_candidates: updatedMatches });
  ...
  const handleDeleteFromCampaign = async () => {
    ...
    try {
      console.log('🗑️ Removing candidate from campaign:', candidate.id);

      // CRITICAL: Fetch fresh campaign data before updating
      console.log('🔄 Fetching fresh campaign data before deleting candidate...');
      const freshCampaigns = await Campaign.filter({ id: campaign.id });
      const freshCampaign = freshCampaigns[0];
      ...
      console.log('✅ Candidate removed from campaign');
            ...
            {stage === 'first_message' && (
              <div className="text-xs p-3 rounded-lg" style={{
                ...
                <p className="font-medium mb-1" style={{ color: '#60A5FA' }}>
                  {user?.language === 'nl' ? '✉️ Bericht in wachtrij' : '✉️ Message queued'}
                </p>
                <p>
                  {getStageDescription()}
                </p>
            ...
            {stage === 'connected' && matchData?.reply_content && (
              ...
              >
                <p className="font-semibold mb-2" style={{ color: '#86EFAC' }}>
                  {user?.language === 'nl' ? '💬 Reactie:' : '💬 Reply:'}
                </p>
