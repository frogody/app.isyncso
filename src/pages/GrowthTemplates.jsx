      ...
      <div className={`min-h-screen ${gt('bg-slate-50', 'bg-black')} p-4`}>
        <div className="max-w-6xl mx-auto space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {[1,2,3,4,5,6].map(i => <Skeleton key={i} className={`h-40 ${gt('bg-slate-200', 'bg-zinc-800')} rounded-xl`} />)}
          </div>
        </div>
      </div>
    );
      ...
      <div className="relative z-10 w-full px-4 lg:px-6 py-4 space-y-4">
        ...
        <div className="relative">
          ...
          <Input
            ...
            className={`pl-10 ${gt('bg-white border-slate-200 text-slate-900', 'bg-zinc-900 border-zinc-700 text-white')}`}
          />
        </div>
                  ...
                  <GlassCard glow="indigo" className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex gap-1">
                        <Button variant="ghost" size="icon" onClick={() => handleEdit(template)} className={`${gt('text-slate-500 hover:text-slate-900', 'text-zinc-400 hover:text-white')} h-8 w-8`}>
                          <Edit className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDelete(template.id)} className={`${gt('text-slate-500', 'text-zinc-400')} hover:text-red-400 h-8 w-8`}>
                          <Trash2 className="w-4 h-4" />
                        </Button>
                      </div>
                    </div>

                    <h3 className={`font-semibold ${gt('text-slate-900', 'text-white')} text-sm mb-2`}>{template.name}</h3>
                    <p className={`text-xs ${gt('text-slate-500', 'text-zinc-400')} line-clamp-2 mb-3`}>{template.description}</p>
        ...
        {showEditor && (
          ...
          >
            <motion.div
              ...
              className={`${gt('bg-white border border-slate-200 shadow-lg', 'bg-zinc-900 border border-zinc-700')} rounded-xl p-4 max-w-lg w-full max-h-[90vh] overflow-y-auto`}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between mb-4">
                <h2 className={`text-lg font-bold ${gt('text-slate-900', 'text-white')}`}>
                  {editingTemplate ? 'Edit Template' : 'New Template'}
                </h2>
                <Button variant="ghost" size="icon" onClick={() => setShowEditor(false)} className={gt('text-slate-500', 'text-zinc-400')}>
                  <X className="w-5 h-5" />
                </Button>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  <label className={`text-xs ${gt('text-slate-500', 'text-zinc-400')} mb-2 block`}>Template Name</label>
                  <Input
                    value={form.name}
                ...
                <div>
                  <label className={`text-xs ${gt('text-slate-500', 'text-zinc-400')} mb-2 block`}>Description</label>
                  <Textarea
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className={`text-xs ${gt('text-slate-500', 'text-zinc-400')} mb-2 block`}>Industry</label>
                    <Input
                      value={form.industry}
                  ...
                  <div>
                    <label className={`text-xs ${gt('text-slate-500', 'text-zinc-400')} mb-2 block`}>Company Size</label>
                    <Input
                ...
                <div>
                  <label className={`text-xs ${gt('text-slate-500', 'text-zinc-400')} mb-2 block`}>Location</label>
                  <Input
                    value={form.location}
                ...
                <div>
                  <label className={`text-xs ${gt('text-slate-500', 'text-zinc-400')} mb-2 block`}>Job Titles (comma-separated)</label>
                  <Input
                ...
                <div>
                  <label className={`text-xs ${gt('text-slate-500', 'text-zinc-400')} mb-2 block`}>Tech Stack (comma-separated)</label>
                  <Input
                ...
                <div>
                  <label className={`text-xs ${gt('text-slate-500', 'text-zinc-400')} mb-2 block`}>Keywords (comma-separated)</label>
                  <Input
                    value={form.keywords}
              ...
              <div className="flex justify-end gap-3 mt-4">
                <Button variant="outline" onClick={() => setShowEditor(false)} className={`${gt('border-slate-200 text-slate-600', 'border-zinc-700 text-zinc-300')}`}>
                  Cancel
                </Button>
