...
import {
  ...
  ChevronRight, Star, Sparkles, GraduationCap, Sun, Moon
} from 'lucide-react';
...
function StatCardEnhanced({ icon: Icon, label, value, sublabel, color = 'teal', onClick, delay = 0 }) {
  ...
  const colorClasses = {
    ...
    violet: { icon: 'text-teal-400/60', bg: lt('bg-slate-100', 'bg-zinc-800/80'), border: lt('border-slate-200', 'border-zinc-700/50'), glow: 'shadow-teal-500/5' },
    emerald: { icon: 'text-teal-400/70', bg: lt('bg-slate-100', 'bg-zinc-800/80'), border: lt('border-slate-200', 'border-zinc-700/50'), glow: 'shadow-teal-500/5' },
  };
  ...
  return (
    <motion.div
      ...
      transition={{ delay }}
      onClick={onClick}
      className={`relative ${lt('bg-white border border-slate-200 shadow-sm', 'bg-zinc-900/50 border border-zinc-800/60')} backdrop-blur-sm rounded-xl p-3 transition-all duration-200 ${
        onClick ? `cursor-pointer hover:border-teal-700/40 ${lt('hover:bg-slate-50', 'hover:bg-zinc-900/70')}` : ''
      }`}
    >
      ...
      {onClick && (
        <div className="absolute bottom-1.5 right-1.5">
          <Info className={`w-2.5 h-2.5 ${lt('text-slate-300', 'text-zinc-700')}`} />
        </div>
      )}
    </motion.div>
  );
  ...
  return (
    ...
    >
      <Link to={createPageUrl(`LessonViewer?courseId=${course.id}`)}>
        <div className={`group relative ${lt('bg-white border border-slate-200 shadow-sm', 'bg-zinc-900/50 border border-zinc-800/60')} backdrop-blur-sm rounded-xl hover:border-teal-800/50 transition-all duration-200 p-3`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h4 className={`font-semibold ${lt('text-slate-900', 'text-zinc-100')} text-sm truncate group-hover:text-teal-300/90 transition-colors`}>
                {course.title}
              </h4>
              <div className="flex items-center gap-2 mt-1.5">
                <Progress value={progress?.completion_percentage || 0} className={`flex-1 h-1.5 ${lt('bg-slate-100', 'bg-zinc-800')}`} />
                <span className="text-xs font-medium text-teal-400/80">{progress?.completion_percentage || 0}%</span>
              </div>
              <div className={`flex items-center gap-2 mt-1.5 text-[10px] ${lt('text-slate-400', 'text-zinc-600')}`}>
                <span className="flex items-center gap-1">
                  <Clock className="w-2.5 h-2.5" />
                  {course.duration_hours}h total
                </span>
                {course.category && (
                  <Badge className={`${lt('bg-slate-100 text-slate-500 border-slate-200', 'bg-zinc-800/80 text-zinc-500 border-zinc-700/50')} text-[9px] px-1 py-0`}>
                    {course.category}
                  </Badge>
                )}
              </div>
            ...
            <Button size="sm" className={`${lt('bg-slate-100 hover:bg-slate-200', 'bg-zinc-800/80 hover:bg-zinc-800')} text-teal-400/80 border ${lt('border-slate-200', 'border-zinc-700/60')} flex-shrink-0`}>
    ...
    return (
      <div className={`min-h-screen ${lt('bg-slate-50', 'bg-black')} p-4`}>
        <div className="space-y-4">
          <Skeleton className={`h-20 w-full ${lt('bg-slate-100', 'bg-zinc-800')} rounded-xl`} />
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[1,2,3,4].map(i => <Skeleton key={i} className={`h-16 ${lt('bg-slate-100', 'bg-zinc-800')} rounded-xl`} />)}
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <Skeleton className={`h-52 ${lt('bg-slate-100', 'bg-zinc-800')} rounded-xl lg:col-span-2`} />
            <Skeleton className={`h-52 ${lt('bg-slate-100', 'bg-zinc-800')} rounded-xl`} />
          </div>
        </div>
      </div>
    );
    ...
    <LearnPageTransition>
      <div className={`min-h-screen ${lt('bg-slate-50', 'bg-black')} relative`}>
        ...
        <div className="relative z-10 w-full px-4 lg:px-6 py-4 space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <GlassCard glow="teal" className="p-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className={`text-sm font-semibold ${lt('text-slate-900', 'text-white')} flex items-center gap-2`}>
                ...
                {continueLearning.length > 0 ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className={`${lt('text-slate-500', 'text-zinc-400')} text-xs mb-3`}>Start your learning journey today</p>
                    <Link to={createPageUrl('Learn')}>
              ...
              {/* Activity Heatmap */}
              <GlassCard glow="teal" className="p-4">
                <h3 className={`text-sm font-semibold ${lt('text-slate-900', 'text-white')} flex items-center gap-2 mb-3`}>
            ...
            <div className="space-y-4">
              {/* Progress Ring */}
              <GlassCard glow="teal" className="p-4">
                <h3 className={`text-sm font-semibold ${lt('text-slate-900', 'text-white')} text-center mb-4`}>Overall Progress</h3>
                ...
                <div className="grid grid-cols-2 gap-3 text-center">
                  <div
                    ...
                    className={`cursor-pointer ${lt('hover:bg-slate-100', 'hover:bg-zinc-800/50')} rounded-lg p-2 transition-colors border border-transparent ${lt('hover:border-slate-200', 'hover:border-zinc-700')}`}
                  >
                    <div className={`text-lg font-bold ${lt('text-slate-900', 'text-white')}`}>{formatMinutes(analytics?.totalTimeMinutes || 0)}</div>
                    <div className={`text-[10px] ${lt('text-slate-500', 'text-zinc-400')} flex items-center justify-center gap-1`}>
              ...
              <GlassCard glow="teal" className="p-4">
                <div
                  ...
                  className={`flex items-center justify-between mb-3 cursor-pointer ${lt('hover:bg-slate-100', 'hover:bg-zinc-800/30')} rounded-lg p-2 -m-2 transition-colors`}
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className={`${lt('text-slate-900', 'text-white')} font-semibold text-sm`}>Level {analytics?.level || 1}</div>
                      <div className={`text-[10px] ${lt('text-slate-500', 'text-zinc-400')}`}>Click for details</div>
                    </div>
                  </div>
                  <ChevronRight className={`w-4 h-4 ${lt('text-slate-400', 'text-zinc-500')}`} />
                </div>

                <div className="mb-3 mt-3">
                  <div className={`flex justify-between text-[10px] ${lt('text-slate-500', 'text-zinc-400')} mb-1`}>
                    <span>Progress to Level {(analytics?.level || 1) + 1}</span>
                    <span className="text-teal-400">{((analytics?.totalPoints || 0) % 1000)} / 1000 XP</span>
                  </div>
                  <Progress value={((analytics?.totalPoints || 0) % 1000) / 10} className={`h-1.5 ${lt('bg-slate-100', 'bg-zinc-800')}`} />
                </div>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className={`${lt('text-slate-900', 'text-white')} font-semibold text-sm`}>{analytics?.streak || 0} Day Streak!</div>
                      <div className={`text-[10px] ${lt('text-slate-500', 'text-zinc-400')}`}>Click to view history</div>
                    </div>
                  </div>
                  <Button
                    size="sm"
        ...
        <Dialog open={showXPModal} onOpenChange={setShowXPModal}>
          <DialogContent className={`${lt('bg-white border-slate-200', 'bg-zinc-900 border-zinc-800')} max-w-md`}>
            <DialogHeader>
            ...
            <div className="space-y-4 mt-4">
              <div className="text-center p-5 rounded-xl bg-gradient-to-br from-teal-500/20 to-teal-600/10 border border-teal-500/30">
                ...
                <div className={`text-sm ${lt('text-slate-500', 'text-zinc-400')}`}>Total XP Earned</div>
              </div>

              <div className="space-y-3">
                <h4 className={`text-sm font-medium ${lt('text-slate-600', 'text-zinc-300')}`}>How to Earn XP</h4>
                {[
                  ...
                  <div key={i} className={`flex items-center justify-between p-3 rounded-lg ${lt('bg-slate-100', 'bg-zinc-800/50')}`}>
                    <div className="flex items-center gap-3">
                      <item.icon className="w-4 h-4 text-teal-400" />
                      <span className={`text-sm ${lt('text-slate-900', 'text-white')}`}>{item.action}</span>
                    </div>
            ...
            <div className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-4 rounded-xl bg-gradient-to-br from-teal-500/20 to-teal-400/10 border border-teal-500/30">
                  ...
                  <div className={`text-sm ${lt('text-slate-500', 'text-zinc-400')}`}>Current Streak</div>
                </div>
                <div className={`text-center p-4 rounded-xl ${lt('bg-slate-100 border border-slate-200', 'bg-zinc-800/50 border border-zinc-700')}`}>
                  <div className={`text-3xl font-bold ${lt('text-slate-900', 'text-white')}`}>{gamification?.longest_streak || analytics?.streak || 0}</div>
                  <div className={`text-sm ${lt('text-slate-500', 'text-zinc-400')}`}>Longest Streak</div>
                </div>
              </div>

              <div className={`p-4 rounded-xl ${lt('bg-slate-100', 'bg-zinc-800/50')}`}>
                <h4 className={`text-sm font-medium ${lt('text-slate-600', 'text-zinc-300')} mb-3`}>Streak Tips</h4>
                <ul className={`space-y-2 text-sm ${lt('text-slate-500', 'text-zinc-400')}`}>
          ...
          <DialogContent className={`${lt('bg-white border-slate-200', 'bg-zinc-900 border-zinc-800')} max-w-md`}>
            ...
            <div className="space-y-4 mt-4">
              <div className="flex items-center justify-center gap-6">
                ...
                <div>
                  <div className={`text-2xl font-bold ${lt('text-slate-900', 'text-white')}`}>Level {analytics?.level || 1}</div>
                  <div className={`text-sm ${lt('text-slate-500', 'text-zinc-400')}`}>{analytics?.totalPoints || 0} total XP</div>
                </div>
              </div>

              <div className={`p-4 rounded-xl ${lt('bg-slate-100', 'bg-zinc-800/50')}`}>
                <div className={`flex justify-between text-sm mb-2`}>
                  <span className={lt('text-slate-500', 'text-zinc-400')}>Progress to Level {(analytics?.level || 1) + 1}</span>
                  <span className="text-teal-400">{((analytics?.totalPoints || 0) % 1000)} / 1000 XP</span>
                </div>
                <Progress value={((analytics?.totalPoints || 0) % 1000) / 10} className="h-3" />
                <div className={`text-xs ${lt('text-slate-400', 'text-zinc-500')} mt-2`}>
                  {1000 - ((analytics?.totalPoints || 0) % 1000)} XP until next level
                </div>
              </div>

              <div className="space-y-2">
                <h4 className={`text-sm font-medium ${lt('text-slate-600', 'text-zinc-300')}`}>Level Rewards</h4>
                {[
                  ...
                  <div key={i} className={`flex items-center justify-between p-3 rounded-lg ${item.unlocked ? 'bg-teal-500/10 border border-teal-500/30' : lt('bg-slate-100', 'bg-zinc-800/50')}`}>
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${item.unlocked ? 'bg-teal-500 text-white' : lt('bg-slate-200 text-slate-500', 'bg-zinc-700 text-zinc-400')}`}>
                        {item.level}
                      </div>
                      <span className={`text-sm ${item.unlocked ? lt('text-slate-900', 'text-white') : lt('text-slate-500', 'text-zinc-400')}`}>{item.reward}</span>
                    </div>
          ...
          <DialogContent className={`${lt('bg-white border-slate-200', 'bg-zinc-900 border-zinc-800')} max-w-md`}>
            ...
            <div className="space-y-4 mt-4">
              <div className="text-center p-5 rounded-xl bg-gradient-to-br from-teal-500/20 to-teal-600/10 border border-teal-500/30">
                ...
                <div className={`text-sm ${lt('text-slate-500', 'text-zinc-400')}`}>Total Learning Time</div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={i} className={`text-center p-3 rounded-lg ${lt('bg-slate-100', 'bg-zinc-800/50')}`}>
                    <div className={`text-xl font-bold ${lt('text-slate-900', 'text-white')}`}>{item.minutes}m</div>
                    <div className={`text-xs ${lt('text-slate-500', 'text-zinc-400')}`}>{item.label}</div>
                  </div>
                ))}
              </div>
