...
export default function ProductsServices() {
  ...
  const handleProductSaved = async (savedProduct) => {
    ...
    setLoading(true);
    try {
      const productsData = await Product.filter({ type: 'service' }, { limit: 500 });
      setProducts(Array.isArray(productsData) ? productsData : []);
      const serviceData = await ServiceProduct.list({ limit: 500 });
      const serviceMap = {};
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        try {
          const result = await Product.filter({ type: 'service' }, { limit: 500 });
        ...
        try {
          const result = await ServiceProduct.list({ limit: 500 });
