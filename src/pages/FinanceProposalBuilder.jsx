      ...
      <FinancePageTransition>
        <div className="p-4 lg:p-6 py-4 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <Skeleton className={`h-[600px] col-span-2 ${ft('bg-slate-200', 'bg-zinc-800')}`} />
            <Skeleton className={`h-[600px] ${ft('bg-slate-200', 'bg-zinc-800')}`} />
          </div>
        </div>
      </FinancePageTransition>
    );
      ...
      <div className="px-4 lg:px-6 py-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button
              ...
              className={ft('text-slate-500', 'text-zinc-400')}
            >
              <ArrowLeft className="w-4 h-4 mr-2" />
              Back
            </Button>
            <div>
              <h1 className={`text-2xl font-bold ${ft('text-slate-900', 'text-white')} flex items-center gap-2`}>
        ...
        <div className="grid grid-cols-3 gap-4">
            ...
            <Card className={ft('bg-white border-slate-200', 'bg-zinc-900/50 border-zinc-800')}>
              <CardContent className="p-4">
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                  <TabsList className={ft('bg-slate-100', 'bg-zinc-800') + ' mb-4'}>
                  ...
                  <TabsContent value="details" className="space-y-4">
                    {/* Title */}
                    <div>
                      <Label className={ft('text-slate-600', 'text-zinc-300')}>Proposal Title *</Label>
                      <Input
                    ...
                    {/* Client Info */}
                    <div className={`grid grid-cols-2 gap-3 p-3 ${ft('bg-slate-100', 'bg-zinc-800/50')} rounded-lg`}>
                      <div className={`col-span-2 flex items-center gap-2 ${ft('text-slate-500', 'text-zinc-400')} text-sm mb-2`}>
                        <Building2 className="w-4 h-4" />
                        Client Information
                      </div>
                      <div>
                        <Label className={`${ft('text-slate-500', 'text-zinc-400')} text-sm`}>Company Name</Label>
                        <Input
                          value={proposal.client_company}
                          onChange={(e) => setProposal(prev => ({ ...prev, client_company: e.target.value }))}
                          className={`${ft('bg-white border-slate-200 text-slate-900', 'bg-zinc-800 border-zinc-700 text-white')} mt-1`}
                          placeholder="Company name"
                        />
                      </div>
                      <div>
                        <Label className={`${ft('text-slate-500', 'text-zinc-400')} text-sm`}>Contact Name</Label>
                        <Input
                      ...
                      <div>
                        <Label className={`${ft('text-slate-500', 'text-zinc-400')} text-sm`}>Email</Label>
                        <Input
                          type="email"
                      ...
                      <div>
                        <Label className={`${ft('text-slate-500', 'text-zinc-400')} text-sm`}>Valid Until</Label>
                        <Input
                          type="date"
                  ...
                  <TabsContent value="content" className="space-y-4">
                    {/* Introduction */}
                    <div>
                      <Label className={ft('text-slate-600', 'text-zinc-300')}>Introduction</Label>
                      <Textarea
                        value={proposal.introduction}
                        onChange={(e) => setProposal(prev => ({ ...prev, introduction: e.target.value }))}
                        className={`${ft('bg-slate-100 border-slate-200 text-slate-900', 'bg-zinc-800 border-zinc-700 text-white')} mt-1 min-h-[120px]`}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <Label className={ft('text-slate-600', 'text-zinc-300')}>Content Sections</Label>
                        <DropdownMenu>
                              ...
                              <div className={`border ${ft('border-slate-200', 'border-zinc-700')} rounded-lg overflow-hidden`}>
                                <CollapsibleTrigger className={`w-full flex items-center justify-between p-3 ${ft('bg-slate-100 hover:bg-slate-200', 'bg-zinc-800/50 hover:bg-zinc-800')} transition-colors`}>
                                  <div className="flex items-center gap-3">
                                    ...
                                    <span className={`${ft('text-slate-900', 'text-white')} text-sm`}>
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <ChevronDown className={`w-4 h-4 ${ft('text-slate-400', 'text-zinc-400')}`} />
                                  </div>
                                </CollapsibleTrigger>
                                <CollapsibleContent>
                                  <div className={`p-3 space-y-3 ${ft('bg-white', 'bg-zinc-900/50')}`}>
                                    <Input
                                      value={section.title}
                                    ...
                                    <Textarea
                                      ...
                                      className={`${ft('bg-slate-100 border-slate-200 text-slate-900', 'bg-zinc-800 border-zinc-700 text-white')} min-h-[100px]`}
                    ...
                    {/* Terms */}
                    <div>
                      <Label className={ft('text-slate-600', 'text-zinc-300')}>Terms & Conditions</Label>
                      <Textarea
                        value={proposal.terms_and_conditions}
                        onChange={(e) => setProposal(prev => ({ ...prev, terms_and_conditions: e.target.value }))}
                        className={`${ft('bg-slate-100 border-slate-200 text-slate-900', 'bg-zinc-800 border-zinc-700 text-white')} mt-1 min-h-[100px]`}
                  ...
                  <TabsContent value="pricing" className="space-y-4">
                      ...
                      {proposal.line_items.length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {proposal.line_items.map((item, idx) => (
                            <div
                              key={idx}
                              className={`flex items-center gap-2 p-3 rounded-lg ${ft('bg-slate-100 border border-slate-200', 'bg-zinc-800/50 border border-white/5')}`}
                            >
                              <div className={`w-8 h-8 rounded flex items-center justify-center ${ft('bg-slate-200/50', 'bg-zinc-700/50')}`}>
                    ...
                    <div className={`grid grid-cols-2 gap-3 p-3 ${ft('bg-slate-100', 'bg-zinc-800/50')} rounded-lg`}>
                      <div>
                        <Label className={`${ft('text-slate-500', 'text-zinc-400')} text-sm`}>Discount</Label>
                        <div className="flex gap-2 mt-1">
                          <Select
                            value={proposal.discount_type}
                            onValueChange={(v) => setProposal(prev => ({ ...prev, discount_type: v }))}
                          >
                            <SelectTrigger className={`w-24 ${ft('bg-white border-slate-200 text-slate-900', 'bg-zinc-800 border-zinc-700 text-white')}`}>
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent className={ft('bg-white border-slate-200', 'bg-zinc-900 border-zinc-700')}>
                              <SelectItem value="percent">%</SelectItem>
                              <SelectItem value="fixed">&euro;</SelectItem>
                            </SelectContent>
                          </Select>
                          <Input
                            type="number"
                      ...
                      <div>
                        <Label className={`${ft('text-slate-500', 'text-zinc-400')} text-sm`}>Tax Rate (%)</Label>
                        <Input
                          type="number"
                  ...
                  <TabsContent value="settings" className="space-y-4">
                    <div className={`flex items-center justify-between p-3 ${ft('bg-slate-100', 'bg-zinc-800/50')} rounded-lg`}>
                      <div>
                        <p className={`${ft('text-slate-900', 'text-white')} font-medium`}>Require Signature</p>
                        <p className={`text-sm ${ft('text-slate-500', 'text-zinc-400')}`}>Client must sign to accept the proposal</p>
                      </div>
                      <Switch
                    ...
                    <div>
                      <Label className={ft('text-slate-600', 'text-zinc-300')}>Currency</Label>
                      <Select
                        value={proposal.currency}
                        onValueChange={(v) => setProposal(prev => ({ ...prev, currency: v }))}
                      >
                        <SelectTrigger className={`${ft('bg-white border-slate-200 text-slate-900', 'bg-zinc-800 border-zinc-700 text-white')} mt-1 w-40`}>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent className={ft('bg-white border-slate-200', 'bg-zinc-900 border-zinc-700')}>
          ...
          <div className="space-y-4">
            <Card className={`${ft('bg-white border-slate-200', 'bg-zinc-900/50 border-zinc-800')} sticky top-4`}>
              <CardHeader className="pb-2">
                <CardTitle className={`${ft('text-slate-900', 'text-white')} flex items-center gap-2 text-sm`}>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  <span className={ft('text-slate-500', 'text-zinc-400')}>Items</span>
                  <span className={ft('text-slate-900', 'text-white')}>{proposal.line_items.length}</span>
                </div>

                {/* Subtotal */}
                <div className="flex justify-between text-sm">
                  <span className={ft('text-slate-500', 'text-zinc-400')}>Subtotal</span>
                  <span className={ft('text-slate-900', 'text-white')}>&euro;{pricing.subtotal.toLocaleString()}</span>
                </div>
                ...
                {pricing.discountAmount > 0 && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-amber-400">-&euro;{pricing.discountAmount.toLocaleString()}</span>
                  </div>
                )}
                ...
                {pricing.taxAmount > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className={ft('text-slate-500', 'text-zinc-400')}>Tax ({proposal.tax_percent}%)</span>
                    <span className={ft('text-slate-900', 'text-white')}>&euro;{pricing.taxAmount.toLocaleString()}</span>
                  </div>
                )}

                <div className={`border-t ${ft('border-slate-200', 'border-zinc-700')} pt-2`}>
                  <div className="flex justify-between items-center">
                    <span className={`text-sm font-medium ${ft('text-slate-900', 'text-white')}`}>Total</span>
                    <span className="text-lg font-bold text-amber-400">
                      &euro;{pricing.total.toLocaleString()}
                    </span>
                  </div>
                </div>
            ...
            <Card className={ft('bg-white border-slate-200', 'bg-zinc-900/50 border-zinc-800')}>
              <CardHeader className="pb-2">
                <CardTitle className={`${ft('text-slate-900', 'text-white')} text-xs`}>Quick Actions</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                <Button
                  variant="outline"
                  size="sm"
                  className={`w-full ${ft('border-slate-200 text-slate-600', 'border-zinc-700 text-zinc-300')} justify-start`}
                  onClick={() => setShowPreview(true)}
                >
                  <Eye className="w-4 h-4 mr-2" />
                  Preview Proposal
        ...
        <Dialog open={showPreview} onOpenChange={setShowPreview}>
          <DialogContent className={`${ft('bg-white border-slate-200 text-slate-900', 'bg-zinc-900 border-zinc-700 text-white')} max-w-3xl max-h-[90vh] overflow-y-auto`}>
                ...
                <div className="border-t border-zinc-200 pt-3">
                  ...
                  <div className="mt-3 space-y-2 text-right">
                    ...
                    <div className="flex justify-end gap-4 text-base font-bold border-t border-zinc-200 pt-2">
                      <span>Total:</span>
                      <span className="w-24 text-amber-600">&euro;{pricing.total.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              )}
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowPreview(false)} className={ft('border-slate-200', 'border-zinc-700')}>
                Close
              </Button>
            </DialogFooter>
          </DialogContent>
