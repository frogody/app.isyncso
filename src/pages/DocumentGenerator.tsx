...
function SystemSelectionCard({
  ...
  return (
    ...
    >
      <SentinelCard variant="interactive" padding="sm" className={isSelected ? st('border-purple-300', 'border-sky-500/30') : ''}>
        {isSelected && (
        ...
        <div className="flex items-start justify-between">
          ...
          <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ml-4', isSelected ? st('bg-purple-100', 'bg-sky-500/20') : st('bg-slate-100', 'bg-zinc-800'))}>
            <Cpu className={cn('w-4 h-4', isSelected ? st('text-purple-500', 'text-sky-400') : st('text-slate-400', 'text-zinc-500'))} />
          </div>
        </div>
      </SentinelCard>
    </motion.div>
    ...
    >
      <SentinelCard variant="interactive" padding="md">
        <div className="flex items-start gap-3 mb-3">
          <div className={cn('w-10 h-10 rounded-lg flex items-center justify-center', st('bg-purple-100', 'bg-sky-500/15'))}>
            <Icon className={cn('w-5 h-5', st('text-purple-500', 'text-sky-400'))} />
          </div>
          <div>
            <h3 className={cn('text-base font-semibold mb-1 transition-colors', st('text-slate-900 group-hover:text-purple-500', 'text-white group-hover:text-sky-400'))}>
              {docType.title}
            </h3>
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          {docType.features.map((feature, i) => (
            <div key={i} className={cn('flex items-center gap-2 text-[10px]', st('text-slate-500', 'text-zinc-600'))}>
              <ChevronRight className={cn('w-3 h-3', st('text-purple-400', 'text-sky-400'))} />
              {feature}
            </div>
          ))}
        </div>
        <div className={cn('flex items-center justify-between pt-3 border-t', st('border-slate-200', 'border-zinc-800/50'))}>
          {docType.aiPowered ? (
            <span className={cn('flex items-center gap-1.5 text-xs', st('text-purple-500', 'text-sky-400'))}>
          ...
          <ArrowRight className={cn('w-4 h-4 transition-colors', st('text-slate-400 group-hover:text-purple-500', 'text-zinc-600 group-hover:text-sky-400'))} />
        </div>
      </SentinelCard>
    </motion.div>
  );
...
export default function DocumentGenerator() {
  ...
  if (error) {
    return (
      <div className={cn('min-h-screen flex items-center justify-center p-6', st('bg-[#F8FAFC]', 'bg-black'))}>
        ...
        <div className="w-full max-w-5xl mx-auto px-4 lg:px-6 py-4 space-y-4">
          ...
          <SentinelCard padding="md">
            <div className="flex items-center gap-3">
              <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center', st('bg-purple-100', 'bg-sky-500/20'))}>
                <Cpu className={cn('w-4 h-4', st('text-purple-500', 'text-sky-400'))} />
              </div>
              <div className="flex-1">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={cn('w-10 h-10 rounded-[20px] flex items-center justify-center', st('bg-purple-100', 'bg-sky-500/10'))}>
              <FileText className={cn('w-5 h-5', st('text-purple-500', 'text-sky-400'))} />
            </div>
            <div>
