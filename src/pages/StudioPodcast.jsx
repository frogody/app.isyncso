            ...
            <div className="flex items-center gap-2">
              <div
                className={`
                  ...
                  ${!isActive && !isCompleted ? `${ct('bg-slate-200', 'bg-zinc-800/60')} ${ct('text-slate-500', 'text-zinc-500')} border ${ct('border-slate-300', 'border-zinc-700/40')}` : ''}
                `}
              >
              ...
              <span
                className={`text-xs font-medium hidden sm:block transition-colors ${
                  isActive ? 'text-yellow-400' : isCompleted ? 'text-yellow-400' : ct('text-slate-500', 'text-zinc-500')
                }`}
              >
                {labels[i]}
              </span>
            ...
            {i < totalSteps - 1 && (
              <div
                className={`w-8 sm:w-12 h-px transition-colors ${
                  isCompleted ? 'bg-yellow-500/40' : ct('bg-slate-300', 'bg-zinc-700/40')
                }`}
              />
            )}
          </React.Fragment>
...
function SelectDropdown({ label, value, onChange, options, icon: Icon }) {
  ...
  return (
    <div className="space-y-2">
      <label className={`text-xs font-medium ${ct('text-slate-500', 'text-zinc-400')} uppercase tracking-wider`}>{label}</label>
      <div className="relative">
        {Icon && (
          <Icon className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${ct('text-slate-400', 'text-zinc-500')} pointer-events-none`} />
        )}
        <select
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className={`w-full ${ct('bg-slate-100', 'bg-zinc-800/60')} border ${ct('border-slate-300', 'border-zinc-700/50')} rounded-xl text-sm ${ct('text-slate-900', 'text-white')} appearance-none cursor-pointer
        ...
        <ChevronRight className={`absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 ${ct('text-slate-400', 'text-zinc-500')} rotate-90 pointer-events-none`} />
      </div>
    </div>
  );
}
...
function DurationPicker({ value, onChange }) {
  ...
  return (
    <div className="space-y-2">
      <label className={`text-xs font-medium ${ct('text-slate-500', 'text-zinc-400')} uppercase tracking-wider`}>Duration</label>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
              value === d.value
                ...
                : `${ct('bg-slate-100', 'bg-zinc-800/40')} ${ct('text-slate-500', 'text-zinc-400')} border ${ct('border-slate-300', 'border-zinc-700/30')} hover:border-zinc-600/50 ${ct('hover:text-slate-700', 'hover:text-zinc-300')}`
            }`}
          >
            {d.label}
          </button>
          ...
          <button
            ...
            className={`relative flex flex-col items-center gap-0.5 px-2 py-2 rounded-lg text-[11px] transition-all ${
              isSelected
                ...
                : `${ct('bg-slate-100', 'bg-zinc-800/30')} border ${ct('border-slate-300', 'border-zinc-700/20')} ${ct('text-slate-500', 'text-zinc-400')} hover:border-zinc-600/40 ${ct('hover:text-slate-700', 'hover:text-zinc-300')}`
            }`}
          >
            {isSelected && (
            ...
            <span className={`text-[10px] ${ct('text-slate-400', 'text-zinc-500')}`}>{v.gender}</span>
          </button>
        );
      })}
    </div>
...
function SpeakerCard({ speaker, index, onUpdate, onRemove, canRemove }) {
  ...
  return (
    <motion.div
      ...
      className={`${ct('bg-white', 'bg-zinc-900/50')} border ${ct('border-slate-200', 'border-zinc-800/60')} rounded-[20px] p-5 space-y-4`}
    >
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className={`text-xs ${ct('text-slate-500', 'text-zinc-500')} font-medium`}>Speaker {index + 1}</span>
            <p className={`text-sm font-semibold ${ct('text-slate-900', 'text-white')}`}>{speaker.name || 'Unnamed'}</p>
          </div>
        </div>
        {canRemove && (
          <button
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <label className={`text-[11px] font-medium ${ct('text-slate-500', 'text-zinc-500')} uppercase tracking-wider`}>Name</label>
          <input
            type="text"
            value={speaker.name}
            onChange={(e) => onUpdate({ ...speaker, name: e.target.value })}
            placeholder="Speaker name"
            className={`w-full ${ct('bg-slate-100', 'bg-zinc-800/50')} border ${ct('border-slate-300', 'border-zinc-700/40')} rounded-lg px-3 py-2 text-sm ${ct('text-slate-900', 'text-white')} ${ct('placeholder:text-slate-400', 'placeholder:text-zinc-600')} focus:outline-none focus:ring-2 focus:ring-yellow-500/30 focus:border-yellow-500/50 transition-all`}
          />
        </div>
        <div className="space-y-1.5">
          <label className={`text-[11px] font-medium ${ct('text-slate-500', 'text-zinc-500')} uppercase tracking-wider`}>Role</label>
          <select
            value={speaker.role}
            onChange={(e) => onUpdate({ ...speaker, role: e.target.value })}
            className={`w-full ${ct('bg-slate-100', 'bg-zinc-800/50')} border ${ct('border-slate-300', 'border-zinc-700/40')} rounded-lg px-3 py-2 text-sm ${ct('text-slate-900', 'text-white')} appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-yellow-500/30 focus:border-yellow-500/50 transition-all`}
          >
            {SPEAKER_ROLES.map((r) => (
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <label className={`text-[11px] font-medium ${ct('text-slate-500', 'text-zinc-500')} uppercase tracking-wider`}>Voice</label>
          <button
            onClick={handlePreviewVoice}
            disabled={previewing}
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium ${ct('bg-slate-100', 'bg-zinc-800/60')} ${ct('text-slate-500', 'text-zinc-400')} hover:text-yellow-400 hover:bg-yellow-500/10 border ${ct('border-slate-300', 'border-zinc-700/30')} hover:border-yellow-500/30 transition-all disabled:opacity-50`}
          >
            {previewing ? (
              <>
...
function AudioPlayer({ audioUrl, duration }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Seek bar */}
      <div className="px-4">
        <div className={`relative h-1.5 ${ct('bg-slate-200', 'bg-zinc-800')} rounded-full cursor-pointer group`} onClick={handleSeek}>
          <motion.div
        ...
        <div className="flex justify-between mt-1.5">
          <span className={`text-[11px] ${ct('text-slate-500', 'text-zinc-500')} font-mono`}>{formatTime(currentTime)}</span>
          <span className={`text-[11px] ${ct('text-slate-500', 'text-zinc-500')} font-mono`}>{formatTime(totalDuration)}</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-3">
        <button
          onClick={() => skip(-15)}
          className={`p-2 rounded-full ${ct('text-slate-500', 'text-zinc-400')} ${ct('hover:text-slate-900', 'hover:text-white')} ${ct('hover:bg-slate-200', 'hover:bg-zinc-800/60')} transition-all`}
        >
    ...
    <div className="space-y-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
      {script.map((line, i) => (
        <div key={i} className="flex gap-3">
          ...
          <p className={`text-sm ${ct('text-slate-600', 'text-zinc-300')} leading-relaxed`}>{line.text}</p>
        </div>
      ))}
    </div>
  );
...
function GenerationProgress({ progress }) {
  ...
  return (
    <div className="space-y-4">
      {/* Progress bar */}
      <div className={`relative h-2 ${ct('bg-slate-200', 'bg-zinc-800')} rounded-full overflow-hidden`}>
        <motion.div
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-sm ${ct('text-slate-600', 'text-zinc-300')}`}>{currentStage.label}...</span>
        </div>
            ...
            <div key={i} className="flex flex-col items-center gap-1">
              <div
                className={`w-2.5 h-2.5 rounded-full transition-all ${
                  reached ? 'bg-yellow-400 shadow-sm shadow-yellow-400/50' : ct('bg-slate-300', 'bg-zinc-700')
                }`}
              />
              <span className={`text-[10px] ${reached ? ct('text-slate-700', 'text-zinc-300') : ct('text-slate-400', 'text-zinc-600')}`}>
                {stage.label}
              </span>
            </div>
          );
...
function Step1TopicScript({
  ...
  return (
    <div className="space-y-6">
      {/* Topic */}
      <div className="space-y-2">
        <label className={`text-xs font-medium ${ct('text-slate-500', 'text-zinc-400')} uppercase tracking-wider`}>
          Podcast Topic / Brief
        </label>
        <textarea
          value={topic}
          ...
          className={`w-full ${ct('bg-slate-100', 'bg-zinc-800/50')} border ${ct('border-slate-300', 'border-zinc-700/40')} rounded-xl px-4 py-3 text-sm ${ct('text-slate-900', 'text-white')} ${ct('placeholder:text-slate-400', 'placeholder:text-zinc-600')} resize-none focus:outline-none focus:ring-2 focus:ring-yellow-500/30 focus:border-yellow-500/50 transition-all`}
        />
      </div>
      ...
      <AnimatePresence>
        {script.length > 0 && (
          ...
          >
            <div className="flex items-center justify-between">
              <label className={`text-xs font-medium ${ct('text-slate-500', 'text-zinc-400')} uppercase tracking-wider flex items-center gap-2`}>
                <FileText className="w-3.5 h-3.5" />
                Generated Script
              </label>
              <span className={`text-[11px] ${ct('text-slate-500', 'text-zinc-500')}`}>{script.length} segments</span>
            </div>
            <textarea
              value={scriptText}
              onChange={(e) => handleScriptEdit(e.target.value)}
              rows={Math.min(16, Math.max(8, script.length * 2))}
              className={`w-full ${ct('bg-slate-100', 'bg-zinc-800/30')} border ${ct('border-slate-300', 'border-zinc-700/30')} rounded-xl px-4 py-3 text-sm ${ct('text-slate-700', 'text-zinc-300')} font-mono leading-relaxed resize-none focus:outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500/40 transition-all custom-scrollbar`}
            />
          </motion.div>
        )}
      </AnimatePresence>
...
function Step2SpeakersVoices({ speakers, setSpeakers }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h3 className={`text-sm font-semibold ${ct('text-slate-900', 'text-white')}`}>Configure Speakers</h3>
          <p className={`text-xs ${ct('text-slate-500', 'text-zinc-500')} mt-0.5`}>{speakers.length} of 4 speakers</p>
        </div>
        <button
          onClick={addSpeaker}
        ...
        {[1, 2, 3, 4].map((n) => (
          <div
            ...
            className={`h-1 flex-1 rounded-full transition-all ${
              n <= speakers.length ? 'bg-yellow-500/60' : ct('bg-slate-200', 'bg-zinc-800/60')
            }`}
          />
        ))}
      </div>
...
function Step3GeneratePreview({ topic, style, tone, duration, speakers, script, user }) {
  ...
  return (
    <div className="space-y-6">
      {/* Summary */}
      <div className={`${ct('bg-white', 'bg-zinc-900/50')} border ${ct('border-slate-200', 'border-zinc-800/60')} rounded-[20px] p-5`}>
        <h3 className={`text-sm font-semibold ${ct('text-slate-900', 'text-white')} mb-4 flex items-center gap-2`}>
          <FileText className="w-4 h-4 text-yellow-400" />
          Podcast Summary
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div>
            <span className={`text-[11px] ${ct('text-slate-500', 'text-zinc-500')} uppercase tracking-wider`}>Style</span>
            <p className={`text-sm ${ct('text-slate-900', 'text-white')} font-medium mt-0.5`}>{styleLabel}</p>
          </div>
          <div>
            <span className={`text-[11px] ${ct('text-slate-500', 'text-zinc-500')} uppercase tracking-wider`}>Tone</span>
            <p className={`text-sm ${ct('text-slate-900', 'text-white')} font-medium mt-0.5`}>{toneLabel}</p>
          </div>
          <div>
            <span className={`text-[11px] ${ct('text-slate-500', 'text-zinc-500')} uppercase tracking-wider`}>Duration</span>
            <p className={`text-sm ${ct('text-slate-900', 'text-white')} font-medium mt-0.5`}>{duration} min</p>
          </div>
          <div>
            <span className={`text-[11px] ${ct('text-slate-500', 'text-zinc-500')} uppercase tracking-wider`}>Speakers</span>
            <p className={`text-sm ${ct('text-slate-900', 'text-white')} font-medium mt-0.5`}>{speakers.length}</p>
          </div>
        </div>
        <div className={`mt-4 pt-4 border-t ${ct('border-slate-200', 'border-zinc-800/40')}`}>
          <span className={`text-[11px] ${ct('text-slate-500', 'text-zinc-500')} uppercase tracking-wider`}>Topic</span>
          <p className={`text-sm ${ct('text-slate-600', 'text-zinc-300')} mt-0.5 line-clamp-2`}>{topic}</p>
        </div>
        {/* Speaker chips */}
        <div className="mt-3 flex flex-wrap gap-2">
          {speakers.map((s) => {
            ...
            return (
              <div
                key={s.id}
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full ${ct('bg-slate-100', 'bg-zinc-800/60')} border ${ct('border-slate-300', 'border-zinc-700/30')} text-xs`}
              >
                <Headphones className="w-3 h-3 text-yellow-400" />
                <span className={`${ct('text-slate-700', 'text-zinc-300')} font-medium`}>{s.name || 'Unnamed'}</span>
                <span className={ct('text-slate-500', 'text-zinc-500')}>as {s.role}</span>
                {voice && <span className={ct('text-slate-400', 'text-zinc-600')}>({voice.name})</span>}
              </div>
            );
          })}
        </div>
      ...
      {generating && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className={`${ct('bg-white', 'bg-zinc-900/50')} border ${ct('border-slate-200', 'border-zinc-800/60')} rounded-[20px] p-6`}
        >
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium ${ct('bg-slate-100', 'bg-zinc-800/60')} ${ct('text-slate-600', 'text-zinc-300')} ${ct('hover:text-slate-900', 'hover:text-white')} border ${ct('border-slate-300', 'border-zinc-700/30')} hover:border-zinc-600/50 transition-all`}
                >
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium ${ct('bg-slate-100', 'bg-zinc-800/60')} ${ct('text-slate-600', 'text-zinc-300')} hover:text-yellow-400 border ${ct('border-slate-300', 'border-zinc-700/30')} hover:border-yellow-500/30 transition-all`}
                >
    ...
    <div className={`min-h-screen ${ct('bg-slate-50', 'bg-black')}`}>
      ...
      <div className="w-full px-4 lg:px-6 py-6 max-w-3xl mx-auto space-y-5">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className={`text-2xl font-bold ${ct('text-slate-900', 'text-white')} mb-1`}>Create Your Podcast</h1>
        ...
        {/* Step Content */}
        <div className={`${ct('bg-white', 'bg-zinc-900/30')} border ${ct('border-slate-200', 'border-zinc-800/40')} rounded-[20px] p-5 sm:p-6 mb-6`}>
        ...
        <div className="flex items-center justify-between">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium ${ct('text-slate-500', 'text-zinc-400')} ${ct('hover:text-slate-900', 'hover:text-white')} ${ct('bg-slate-100', 'bg-zinc-800/40')} border ${ct('border-slate-300', 'border-zinc-700/30')} hover:border-zinc-600/50 transition-all disabled:opacity-0 disabled:pointer-events-none`}
          >
