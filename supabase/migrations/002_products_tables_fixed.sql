...
-- FIXED: RLS policies with proper WITH CHECK clauses for INSERT
...
-- ============================================================================
-- RLS POLICIES (FIXED with WITH CHECK for INSERT support)
-- ============================================================================
...
DROP POLICY IF EXISTS "Users can view suppliers for their company" ON public.suppliers;
DROP POLICY IF EXISTS "Admins can manage suppliers" ON public.suppliers;
DROP POLICY IF EXISTS "Users can view categories for their company" ON public.product_categories;
DROP POLICY IF EXISTS "Admins can manage categories" ON public.product_categories;
DROP POLICY IF EXISTS "Users can view products for their company" ON public.products;
DROP POLICY IF EXISTS "Admins can manage products" ON public.products;
DROP POLICY IF EXISTS "Users can view digital products" ON public.digital_products;
DROP POLICY IF EXISTS "Admins can manage digital products" ON public.digital_products;
DROP POLICY IF EXISTS "Users can view physical products" ON public.physical_products;
...
-- Suppliers: Full access for users in the same company
...
-- Product Categories: Full access for users in the same company
CREATE POLICY "Users can manage categories for their company" ON public.product_categories
  FOR ALL
  USING (company_id IN (SELECT company_id FROM public.users WHERE id = auth.uid()))
  WITH CHECK (company_id IN (SELECT company_id FROM public.users WHERE id = auth.uid()));

-- Products: Full access for users in the same company
CREATE POLICY "Users can manage products for their company" ON public.products
  FOR ALL
  USING (company_id IN (SELECT company_id FROM public.users WHERE id = auth.uid()))
  WITH CHECK (company_id IN (SELECT company_id FROM public.users WHERE id = auth.uid()));

-- Digital Products: Full access for users whose company owns the parent product
...
-- Physical Products: Full access for users whose company owns the parent product
